...
class SharedLatentGain(Encoder):
    def __init__(self, tv_dims, 
        ...
        if include_tv:
            self.tv = layers.NDNLayer(input_dims=[tv_dims, 1, 1, 1],
        ...
        ''' neuron gain '''
        if include_gain:
            self.gain_mu = layers.NDNLayer(input_dims=[1, 1, 1, num_trials], num_filters=num_latent, 
                NLtype='lin',
                bias=False,
        ...
        ''' neuron offset '''
        if include_offset:
            self.offset_mu = layers.NDNLayer(input_dims=[1, 1, 1, num_trials], num_filters=num_latent, 
                NLtype='lin',
                bias=False,
