...
from data import get_psths
from fitlvm_utils import check_stable_lowd, get_data_model, fit_model, fit_autoencoder, fit_gain_model, eval_model
from models import *
...
'''
class LVMFitter:
    def __init__(self, trial_data=None, spike_times=None, session_data=None, regions=None, **kwargs):
        '''
        kwargs:
        - tpre
        - tpost
        ...
        # model params
        self.task_vars = kwargs.pop('task_vars', ['response', 'rewarded', 'block_side', 'response_prev', 'rewarded_prev'])
        ...
        self.n_latents = kwargs.pop('n_latents', 1)
    ...
    def get_data(self):
        self.psths, self.trial_mask = get_psths(self.spike_times, self.trial_data, self.session_data, self.regions, 
                                                                tpre=self.tpre, tpost=self.tpost, binwidth_ms=self.binwidth_ms, 
                                                                alignment=self.alignment, get_strategy=False)
        self.trial_data = self.trial_data[self.trial_mask]
        
        self.data_gd, self.train_dl, self.val_dl, self.test_dl, self.indices, self.num_trials, self.num_tv, self.num_units = get_data_model(self.psths, 
                                                                                                                              self.trial_data, 
                                                                                                                              self.regions, 
    ...
    def get_cids(self):
        ...
        self.cids_pca = check_stable_lowd(self.data_gd, self.train_dl.dataset, self.val_dl.dataset, self.num_units, rank=4)
    ...
    def fit_ae_gain(self): 
        ...
        if self.n_splines > 1:
            self.mod_ae_gain.drift.weight.data = self.mod_taskvar.drift.weight.data[:,self.cids].clone()
            self.mod_ae_gain.bias.requires_grad = False
        else:
            self.mod_ae_gain.bias.requires_grad = True
        
        self.mod_ae_gain.tv.weight_data          = self.mod_taskvar.tv.weight.data[:,self.cids].clone()
        ...
        fit_autoencoder(self.mod_ae_gain, self.train_dl, self.val_dl, 
                        min_iter=0, max_iter=self.max_iters)
    ...
    def fit_ae_offset(self):
        ...
        if self.n_splines > 1:
            self.mod_ae_offset.drift.weight.data = self.mod_taskvar.drift.weight.data[:,self.cids].clone()
            self.mod_ae_offset.bias.requires_grad = False
        else:
            self.mod_ae_offset.bias.requires_grad = True
        
        self.mod_ae_offset.tv.weight_data          = self.mod_taskvar.tv.weight.data[:,self.cids].clone()
        self.mod_ae_offset.bias.data               = self.mod_taskvar.bias.data[self.cids].clone()
        self.mod_ae_offset.tv.weight.requires_grad = False
        
        self.mod_ae_offset.readout_gain.weight_scale   = 1.0
        self.mod_ae_offset.latent_gain.weight_scale    = 1.0
        self.mod_ae_offset.readout_gain.weight.data[:] = 1.0
        
        self.mod_ae_offset.prepare_regularization()
        
        fit_autoencoder(self.mod_ae_offset, self.train_dl, self.val_dl, 
                        min_iter=0, max_iter=self.max_iters)
    
    def fit_ae_affine(self):
        self.mod_ae_affine = SharedGain(
            num_tv=self.num_tv,
            num_unit=self.num_units,
            cids=self.cids,
            num_latent=self.n_latents,
            num_tents=self.n_splines,
            latent_noise=self.add_latent_noise,
        ...
        if self.n_splines > 1:
            self.mod_ae_affine.drift.weight.data = self.mod_taskvar.drift.weight.data[:,self.cids].clone()
        ...
        self.mod_ae_affine.tv.weight_data          = self.mod_taskvar.tv.weight.data[:,self.cids].clone()
        ...
        self.mod_ae_affine.readout_gain.weight.data[:] = self.mod_ae_gain.readout_gain.weight.data.detach().clone() # .shape >> torch.Size([3, 173])
        self.mod_ae_affine.readout_offset.weight.data[:] = self.mod_ae_offset.readout_offset.weight.data.detach().clone()
        
        self.mod_ae_affine.latent_gain.weight.data[:] = self.mod_ae_gain.latent_gain.weight.data.detach().clone() # .shape >> torch.Size([173, 3])
        self.mod_ae_affine.latent_offset.weight.data[:] = self.mod_ae_offset.latent_offset.weight.data.detach().clone()
        ...
        fit_autoencoder(self.mod_ae_affine, self.train_dl, self.val_dl, 
                        min_iter=0, max_iter=self.max_iter)
    ...
    def eval(self):
        ...
        self.mod_baseline.drift.weight.data = self.mod_baseline.drift.weight.data[:,self.cids]
        self.mod_baseline.drift.bias.data = self.mod_baseline.drift.bias.data[self.cids]
        
        self.res_baseline = eval_model(self.mod_baseline, self.data_gd, self.test_dl.dataset)
        
        # task variables
        self.mod_taskvar.cids = self.cids
        self.mod_taskvar.bias.data = self.mod_taskvar.bias.data[self.cids]
        self.mod_taskvar.drift.weight.data = self.mod_taskvar.drift.weight.data[:,self.cids]
        ...
        self.res_taskvar = eval_model(self.mod_taskvar, self.data_gd, self.test_dl.dataset)
        
        # lvms
        self.res_gain = eval_model(self.mod_gain, self.data_gd, self.test_dl.dataset)
        self.res_offset = eval_model(self.mod_offset, self.data_gd, self.test_dl.dataset)
        self.res_affine = eval_model(self.mod_affine, self.data_gd, self.test_dl.dataset)
