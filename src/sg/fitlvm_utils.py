...
# PLOTTING
def plot_summary(das, block_side,  subj_idx, sess_idx, model_name='affineae_lvm', sort_by="gain", save=False, fpath=None):
    ...
    for latent_idx in range(n_latents): #gain_mu.get_weights().shape[1]):
        ...
        if "gain" in model_name or "affine" in model_name: rhog = spearmanr(block_side, zgain)
        if "offset" in model_name or "affine" in model_name: rhoo = spearmanr(block_side, zoffset)
